...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.909876, 110.162983], 13);
  ...
  <?php
    $arrayPanjatan = [];
    $getPanjatan = $db->ObjectBuilder()->get('dusun',1);
    $row = $getPanjatan[0];
    foreach ($getPanjatan as $row) {
  ?>
  ...
  <?php
      $arrayPanjatan[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/panjatan.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Panjatan",
      layers: [<?=implode(',', $arrayPanjatan);?>]
    }
  ];
  ...
  <?php
  $getPanjatan = $db->ObjectBuilder()->get('dusun');
?>
