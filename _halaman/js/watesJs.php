...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.889902, 110.136966], 13);
  ...
  <?php
    $arrayWates = [];
    $getWates = $db->ObjectBuilder()->get('dusun',1);
    $row = $getWates[0];
    foreach ($getWates as $row) {
  ?>
  ...
  <?php
      $arrayWates[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/wates.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Wates",
      layers: [<?=implode(',', $arrayWates);?>]
    }
  ];
  ...
  <?php
  $getWates = $db->ObjectBuilder()->get('dusun');
?>
