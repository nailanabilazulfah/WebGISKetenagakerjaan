...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.9404603,110.2078972], 13);
  ...
  <?php
    $arrayGalur = [];
    $getGalur = $db->ObjectBuilder()->get('dusun',1);
    $row = $getGalur[0];
    foreach ($getGalur as $row) {
  ?>
  ...
  <?php
      $arrayGalur[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/'.$row->geojson_dusun.'"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Galur",
      layers: [<?=implode(',', $arrayGalur);?>]
    }
  ];
  ...
  <?php
  $getGalur = $db->ObjectBuilder()->get('dusun');
?>
