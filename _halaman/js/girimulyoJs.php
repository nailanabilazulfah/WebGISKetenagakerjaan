...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.7448265,110.155807], 13);
  ...
  <?php
    $arrayGirimulyo = [];
    $getGirimulyo = $db->ObjectBuilder()->get('dusun',1);
    $row = $getGirimulyo[0];
    foreach ($getGirimulyo as $row) {
  ?>
  ...
  <?php
      $arrayGirimulyo[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/girimulyo1.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Girimulyo",
      layers: [<?=implode(',', $arrayGirimulyo);?>]
    }
  ];
  ...
  <?php
  $getGirimulyo = $db->ObjectBuilder()->get('dusun');
?>
