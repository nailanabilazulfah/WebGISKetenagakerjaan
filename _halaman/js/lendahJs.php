...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.916487, 110.228119], 13);
  ...
  <?php
    $arrayLendah = [];
    $getLendah = $db->ObjectBuilder()->get('dusun',1);
    $row = $getLendah[0];
    foreach ($getLendah as $row) {
  ?>
  ...
  <?php
      $arrayLendah[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/lendah.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Lendah",
      layers: [<?=implode(',', $arrayLendah);?>]
    }
  ];
  ...
  <?php
  $getLendah = $db->ObjectBuilder()->get('dusun');
?>
