...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.6893339,110.227063], 13);
  ...
  <?php
    $arrayKalibawang = [];
    $getKalibawang = $db->ObjectBuilder()->get('dusun',1);
    $row = $getKalibawang[0];
    foreach ($getKalibawang as $row) {
  ?>
  ...
  <?php
      $arrayKalibawang[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/kalibawang.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Kalibawang",
      layers: [<?=implode(',', $arrayKalibawang);?>]
    }
  ];
  ...
  <?php
  $getKalibawang = $db->ObjectBuilder()->get('dusun');
?>
