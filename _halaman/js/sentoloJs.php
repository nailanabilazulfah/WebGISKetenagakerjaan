...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.854009, 110.215290], 12);
  ...
  <?php
    $arraySentolo = [];
    $getSentolo = $db->ObjectBuilder()->get('dusun',1);
    $row = $getSentolo[0];
    foreach ($getSentolo as $row) {
  ?>
  ...
  <?php
      $arraySentolo[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/sentolo.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Sentolo",
      layers: [<?=implode(',', $arraySentolo);?>]
    }
  ];
  ...
  <?php
  $getSentolo = $db->ObjectBuilder()->get('dusun');
?>
