...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.678922, 110.169522], 13);
  ...
  <?php
    $arraySamigaluh = [];
    $getSamigaluh = $db->ObjectBuilder()->get('dusun',1);
    $row = $getSamigaluh[0];
    foreach ($getSamigaluh as $row) {
  ?>
  ...
  <?php
      $arraySamigaluh[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/samigaluh.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Samigaluh",
      layers: [<?=implode(',', $arraySamigaluh);?>]
    }
  ];
  ...
  <?php
  $getSamigaluh = $db->ObjectBuilder()->get('dusun');
?>
