...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.825011, 110.149702], 12);
  ...
  <?php
    $arrayPengasih = [];
    $getPengasih = $db->ObjectBuilder()->get('dusun',1);
    $row = $getPengasih[0];
    foreach ($getPengasih as $row) {
  ?>
  ...
  <?php
      $arrayPengasih[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/pengasih.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Pengasih",
      layers: [<?=implode(',', $arrayPengasih);?>]
    }
  ];
  ...
  <?php
  $getPengasih = $db->ObjectBuilder()->get('dusun');
?>
