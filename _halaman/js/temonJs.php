...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.886831, 110.072175], 13);
  ...
  <?php
    $arrayTemon = [];
    $getTemon = $db->ObjectBuilder()->get('dusun',1);
    $row = $getTemon[0];
    foreach ($getTemon as $row) {
  ?>
  ...
  <?php
      $arrayTemon[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/temon.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Temon",
      layers: [<?=implode(',', $arrayTemon);?>]
    }
  ];
  ...
  <?php
  $getTemon = $db->ObjectBuilder()->get('dusun');
?>
