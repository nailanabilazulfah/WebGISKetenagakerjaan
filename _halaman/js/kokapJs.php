...
<script type="text/javascript">
  ...
  var mymap = L.map('maps').setView([-7.829512,110.0946305], 13);
  ...
  <?php
    $arrayKokap = [];
    $getKokap = $db->ObjectBuilder()->get('dusun',1);
    $row = $getKokap[0];
    foreach ($getKokap as $row) {
  ?>
  ...
  <?php
      $arrayKokap[] = '{
        ...
        layer: new L.GeoJSON.AJAX(["assets/unggah/dusun_geojson/kokap.geojson"], {
  ...
  var overLayers = [
    {
      group: "Layer Kecamatan Kokap",
      layers: [<?=implode(',', $arrayKokap);?>]
    }
  ];
  ...
  <?php
  $getKokap = $db->ObjectBuilder()->get('dusun');
?>
